...
void MSDRadixSort::sort(std::vector<std::string>& data, size_t& charComparisons) {
    charComparisons = 0;
...
int MSDRadixSort::charAt(const std::string& s, int d, size_t& cmp) {
    ++cmp;
    return d < s.size() ? static_cast<unsigned char>(s[d]) : -1;
}

void MSDRadixSort::msdSort(std::vector<std::string>& data, int low, int high, int d, size_t& cmp, std::vector<std::string>& aux) {
    if (high <= low) return;
...
int main() {
    ...
    MSDRadixSort::sort(strings, comparisons);
